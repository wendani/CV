...
\item
  ...
  when a packet hits entries in both IPv4/IPv6 and L2 tables, which are installed in different TCAM sets.
  ...
  \resitem{Implemented PFC pause status get in HSDK 6.5.25 to support queue pause status query in SAI.
  ...
  \resitem{Designed and implemented LAG egress ACL bind under the notion of interface class port bitmap on TH4 and L2 interface on TD4.}
  ...
  \resitem{Proposed and validated single buffer pool design that enables traffic-hitless buffer profile change (from lossy to lossless, and vice versa).}
  ...
  which change to profile-id-based map attachment to port.}
  \resitem{Hardware PFC watchdog.}
  ...
  \resitem{ACL user defined field (UDF)}
  \end{itemize}
...
Wiley Networks Journal (1), Wiley Transactions on Emerging Telecommunications Technologies (1), IEEE China Communications (1),
