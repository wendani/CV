...
13804 174th PL NE&  \\
Redmond, WA 98052  & M: +1 (425) 984 9332\\
...
\item
  ...
  \resitem{\textcolor{Red}{2020 annual performance: ByteStyle E (exceed expectations)---``An excellent role model for team and company'';
  Engagement E (exceed expectations)---``Highly engaged; has strong ownership as a role model''.}}
  ...
  via the notion of logical tables on Trident3 to double the number of ACL tables user can create at ingress that
  ...
  Enabled logical tables on Trident3 for TCAM slice sharing among mutually exclusive field groups (74 insertions, 7 deletions);
  ...
  \resitem{Enabled truncation of mirrored packet (fixed truncation size on Trident3).
  ...
  \resitem{Unlocked ERSPAN IPv6 underlay capability on Trident3 SDK 6.5.16, which requires CANCUN upgrade from v5.3.3 to v6.1.3, together with SDK patch.
  ...
  \resitem{Enhanced sub port router interface: Fix kernel admin status, and developed vs unit test
  ...
  \resitem{Hacked SDK to enable fine-grained TCAM slice sharing among ACL tables of the same type (e.g., among mutually exclusive L3 tables).
  Fixed 3 SDK bugs on ACL logical tables.
  Hacked SDK to bypass hardware limitation on the number of ACL tables user can create (32 on Trident3)
  via logical table select TCAM entry sharing among ACL tables of the same type and priorty.
...
\item
\textbf{Reviewer} of IEEE/ACM Transactions on Networking (1), IEEE/OSA Journal of Lightwave Technology (8), IEEE Journal on Selected Areas in Communications (1),
