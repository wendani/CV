...
\item
  ...
  \begin{itemize}
  \resitem{Topics: Lambda network operating system over world's bleeding-edge switching chips (25G, 100G, 400G)}
  \resitem{LAG sub port router interface}
  \resitem{config file, default S0 (ToR) switch config auto gen}
  \resitem{buffer \& QoS profile for RDMA and non-RDMA traffic}
  \resitem{ntp service in default VRF over loopback interface}
  \resitem{Proposed host interface ICMP and ICMPv6 trap types in SAI}
  \resitem{Brought up Celestica Questone platform; Develop fan control and monitoring script}
  \resitem{Brought up Quanta IX8E-56X, Ruijie B6510-48VS8CQ platforms; save the system from kernel panic and oops}
  \resitem{Developed standard workflow to code submission and to embrace community code updates;
  Pioneered and accomplished the first code updates to synchronize with the community 201911 branch, filling a total gap of around 180 commits behind}
  \resitem{Broadcom SAI 3.6.7.1 (OCP SAI 1.4.1, Broadcom SDK 6.5.16) running on SONiC 201911 branch image
  (Broadcom SAI 3.7.3.3-2, OCP SAI 1.5.1, Broadcom SDK 6.5.16):
  ...
  as its running environment relies on the libprotobuf10 library (3.0.0-9), which is not available on earlier Debian 8 release---Jessie);
  Debugged syncd crash on original Broadcom SAI release, and fixed SAI codes.}
  \resitem{Upgraded OCP SAI header to v1.5.1, v1.5.2, and enabled image build against Broadcom SAI 3.6.7.1}
  \resitem{SAI change without support from Broadcom: Suppressed global pause ability advertisement on Trident3 server-facing ports;
  Implemented host interface ICMP and ICMPv6 trap, rate limiting, and counter attachment.}
  \resitem{Designed and implemented VLAN bind point support for egress ACL table, from command line interface (CLI) down to Broadcom SAI;
...
\item
  ...
  \resitem{Awarded two tickets to Seattle Sounders match in the ScottGu Cloud + AI Red Zone Suites for going above and beyond in performance}
  \end{itemize}
...
\item[C++:] {\tt mutable}, {\tt struct}, constructor (top-down, inside out), member initializer list, {\tt explicit}, direct initialization, copy initialization,
...
\item[Python:] sequence type ({\tt len}), list ({\tt remove}), tuple, mapping type, dictionary ({\tt items}, {\tt get}, {\tt setdefault}, {\tt pop}),
...
except, finally, module, argparse, pickle, re ({\tt compile}, {\tt match}), subprocess, binascii, click

\item[Scripts:] bash, tcsh, expansion (variable, arithmetic), command substitution, subshell, process substitution \textless(), {\tt read},
{\tt eval}/` `,
{\tt if then}, {\tt for do}, {\tt while do}, {\tt case in} (;;), {\tt test}/[ ], [[ ]], \&\&, $||$, \$?, \$\#, \$!, set (-x, +x, -{}-),
...
UDP, DHCP (RFC 2131), ISC DHCP, SNMP (agent, ASN.1 encoding/decoding, trap, mib), SNTPv4 (RFC 4330), NTPv4 (RFC 5905),
socket programming, NicheStack\textsuperscript{TM} IPv4 (arp, udp, snmpv2c, sntpv4),
%
...
buffer watermark, buffer config manager, ACL, warm-reboot, scheduler, QoS mapping, neighbor, route, copp, critical resource management (CRM),
virtual switch (vs)),
%
OCP switch abstraction interface (SAI) (pipeline, buffer, host interface, Broadcom implementation, python-saithrift rpc library),
...
\item[Applications:] code reading, regular expression (basic, extended, [:space:], [:alnum:]), system debugging, Linux kernel programming,
...
redis (-n, -{}-scan, -{}-pattern, {\tt get}, {\tt hget}, {\tt set}, {\tt hset}, {\tt del}, {\tt hdel} (multiple fields), dump (-d, -k, -y), load),
Broadcom SDK (BST, KNET, QoS mapping, FP, flex counters, scheduler, LAG, hashing), Cancun, flex flow APIs, OpenNSL,
Azure ExpressRoute
...
\item[Tools:] GNU Makefile (expansion (recursive =, simple :=, secondary, conditional ?=), {\tt eval}, {\tt wildcard}, phony target,
conditional, {\tt strip}, conditional expansion {\tt if}),
gdb, crash, gprof, OProfile, {\tt iperf},
strace, ftrace, SystemTap,
Scapy (ether, dot1q, ip, arp, icmp), Wireshark, autoconf (./autogen.sh, ./configure, make, make install), cygwin, cross toolchain (OpenWrt, Altera),
JTAG, SnmpB,
...
{\tt ps},
{\tt mkfs}, {\tt less}, {\tt tail} (-n), {\tt grep} (-r, -i, -n, -v, -q, -{}-color), {\tt find} (-type), {\tt xargs}, {\tt ln}, {\tt augtool} (-r, set),
{\tt mount}, {\tt unmount}, /etc/fstab,
{\tt file}, {\tt chmod} (+x), {\tt nm}, {\tt tree},
{\tt exec}, {\tt wait}, {\tt trap},
{\tt export}, {\tt source}/.,
{\tt tar} (.tar, .tar.gz), {\tt gzip}, {\tt gunzip} (.gz), {\tt zcat}, {\tt zgrep},
{\tt ifconfig}, {\tt ifupdown2}, {\tt ip addr}, {\tt ip link}, {\tt ethtool}, {\tt arp}, {\tt ip neigh}, {\tt ip link},
{\tt tcpdump}, {\tt ip route}, {\tt ip rule},
{\tt wget}, {\tt curl} (-o), {\tt md5sum},
{\tt chown}, {\tt disown}, {\tt adduser}, {\tt usermod}, {\tt passwd}, {\tt id},
{\tt dpkg}, {\tt dpkg-buildpackage} (-nc), {\tt fakeroot}, {\tt debhelper} (dh\_clean),
{\tt which},
{\tt lsusb},
{\tt virsh},
{\tt diff}, {\tt comm},
{\tt echo} (-n), {\tt tee}, {\tt cat} (-A, ctrl-D, \textless\textless token), {\tt awk} (/pattern/action), {\tt cut} (-d, -f, -c),
...
Celestica Fishbone-48, Seastone-DX010, Questone,
Quanta IX8C-56X, IX8E-56X, Ruijie RG-B6510-48VS8CQ, H3C TCS81-120F,
...
flexible pipeline, memory management unit, buffer management, buffer statistics tracking, oversubscription buffer,
...
Dockerfile (FROM, RUN, COPY; WORKDIR, EXPOSE, ENTRYPOINT), build (-t),
...
\item[Version Control:] Git (local, remote, add -{}-patch, branch -{}-contains, tag -a,
fetch -{}-tags, push -{}-force -{}-delete, show, rebase, apply, cherry-pick, submodule update -{}-recursive, bisect, blame -{}-line-porcelain, archive),
GitHub workflow, GitLab workflow, Bitbucket, Visual Studio Team Services,
...
\item
\textbf{Reviewer} of IEEE/ACM Transactions on Networking (1), IEEE/OSA Journal of Lightwave Technology (7),
