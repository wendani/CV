...
C++ ({\tt explicit}, direct initialization, copy initialization, {\tt cast}, squential container (emplace),
string (NULL, rational operators, {\tt empty}, {\tt substr}, {\tt stoul}),
associative container, map (at, count), unordered\_map, implicit interface, virtual function \& default argument, shared\_ptr, unique\_ptr, lambda expression, member initializer),
Objective-C,
Python (sequence type (list, tuple), mapping type (dictionary), sorted, map, lambda, isinstance, ternary, not, in, import, from, parameter passing,
global, nested function, scoping, assert,
finally, module, argparse, pickle, re, subprocess, os),
Jinja2 (template, custom filter, macro, include), YAML (sequence, mapping, scalar),
...
Software for Open Networking in the Cloud (SONiC), OCP switch abstration interface (SAI),
FEC (Reed-Solomon)
...
\item[Applications:] regular expression (basic, extended), Linux kernel programming, Linux system programming ({\tt select}, {\tt getenv}), interprocess communication (IPC), process scheduling,
...
decorator (add responsibilities incrementally \& dynamically), singleton), Kusto query,
...
switch chip (Broadcom BCM56850 (Trident2), BCM56970 (Tomahawk2), BCM56960 (Tomahawk), memory management unit, buffer management, buffer statistics tracking,
cross-point (XP) architecture, PFC, ECN, WRED, BCM88670 (Jericho), packet processor architecture),
...
\item[Automation:] Ansible (ansible-playbook, name, with\_items, when, include\_vars, register, become, connection,
...
vinyl cutting (Silhouette Cameo), Raspberry Pi Zero W, Arduino Uno (button, LED, potentiometer, speaker)
