...
\item
  ...
  Resolved the issue of PFC watchdog not started after reboot by fixing the initialization dependancy.}
...
\item
  ...
  \resitem{Built a solid foundation on Broadcom XGS switch chip pipeline from a exhaustive study of Broadcom University training materials
...
destructor (bottom-up, outside in), address of reference, operator(), operator type,
cast ({\tt dynamic\_cast}), {\tt swap},
sequential container ({\tt empty}, {\tt size}, operator[ ], operator=, {\tt front}, {\tt emplace\_back},
{\tt push\_back}, {\tt pop\_back}, {\tt erase}, {\tt clear}),
string ({\tt NULL}, rational operators, {\tt find}, {\tt substr}, {\tt stoul}, {\tt npos}),
vector ({\tt data}, {\tt capacity}, {\tt reserve}),
associative container ({\tt size}, {\tt find}, {\tt count}, {\tt emplace}, {\tt empty}, {\tt insert}, {\tt erase}),
pair, map ({\tt at}), unordered\_map, set ({\tt clear}),
...
XML, AMPL, MATLAB, script (bash, tcsh, expansion (variable, arithmetic), command substitution, subshell, process substitution \textless(), {\tt read},
...
buffer watermark, buffer config manager, ACL, warm-reboot, scheduler, QoS mapping),
OCP switch abstraction interface (SAI) (Broadcom, python-saithrift rpc library)
FEC (Reed-Solomon)
...
\item[Applications:] regular expression (basic, extended, [:space]), system debugging, Linux kernel programming,
Linux system programming ({\tt select}, {\tt poll}, {\tt epoll}, {\tt getenv}, timerfd), netlink,
