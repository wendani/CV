...
\item
  ...
  on Broadcom BCM56850 and BCM56970 switch chips
...
\item[Programming Languages:] C ({\tt const}, {\tt volatile}), C++ in C, assembly (Nios II, x86 \& x64 (Intel \& AT\&T syntaxes), MicroBlaze, ARM),
inline assembly (gcc), C in C++, Java,
C++ ({\tt explicit}, direct initialization, copy initialization, {\tt cast}, squential container, string (NULL, rational operators, {\tt stoul}),
associative container, implicit interface, virtual function \& default argument),
Objective-C,
Python (sequence type (list, tuple), mappying type (dictionary), sorted, lambda, ternary, not), Jinja2 (template, custom filter), YAML (sequence, mapping, scalar),
XML, AMPL, MATLAB, script (bash, tcsh), Fortran, Verilog, VHDL, SQL %
...
\item[Applications:] Linux kernel programming, Linux system programming ({\tt select}, {\tt getenv}), interprocess communication (IPC), process scheduling,
...
{\tt ps},
{\tt mkfs}, {\tt grep} \& regular expression, {\tt find}, {\tt less}, {\tt tail}, {\tt ln}, {\tt mount}, /etc/fstab, {\tt file}, {\tt chmod},
{\tt tar} (.tar, .tar.gz), {\tt gzip}, {\tt gunzip} (.gz), {\tt zcat},
{\tt ifconfig}, {\tt ip addr}, {\tt ip link}, {\tt ethtool}, {\tt arp}, {\tt ip neigh}, {\tt ip link}, {\tt tcpdump}, {\tt ip route}, {\tt wget},
...
switch (Arista 7124SX, 7050QX-32S, 7260CX3-64, Dell S6100-ON, Mellanox SN2700, Ubiquiti EdgeSwitch16, Netgear M4200, GS108PE, GS110TP), PCIe, SR-IOV, DMA,
...
cross-point (XP) architecture, PFC, ECN, WRED),
...
build context, Dockerfile (FROM, RUN, COPY; WORKDIR, EXPOSE, ENTRYPOINT),
load, images, tag, image (ls, history, inspect, build),
ps, run, exec, stop, start, attach,
cp)

\item[Automation:] Ansible (ansible-playbook)

\item[Version Control:] Git (local, remote, apply, cherry-pick, submodule update, git bisect), GitHub workflow, Bitbucket, Visual Studio Team Services,
CVS, TortoiseCVS, SVN, TortoiseSVN, Source Depot
...
\item[Garage:] 3D printing (Ultimaker 3 Extended, Cura)
