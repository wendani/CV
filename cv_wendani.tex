...
\item
  ...
  {Software engineer \Romannum{2}}{Jan. 2019 - present}
  \begin{itemize}
  \resitem{Topics: white-box data center switches (40G, 50G, 100G)}
  \end{itemize}

\item
  \ressubheading{Microsoft Corporation}{Vancouver, BC, Canada}
  {Software engineer 2}{Dec. 2017 - Jan. 2019}
  \begin{itemize}
  \resitem{Topics: white-box data center switches (40G, 50G, 100G)}
  \resitem{Characterized the DSCP mapping, ingress admission control (flow control XOFF/XON threshold, pool size),
  egress admission control (drop state, pool size), ECN marking, and WRED dropping behavior of memory management unit (MMU)
  on Broadcom BCM56850, BCM56960, and BCM56970 switch chips
...
\item[Programming Languages:] C ({\tt const}, {\tt volatile}, strstr), C++ in C, assembly (Nios II, x86 \& x64 (Intel \& AT\&T syntaxes), MicroBlaze, ARM),
inline assembly (gcc), C in C++, Java,
...
destructor (bottom-up, outside in),
cast ({\tt dynamic\_cast}), {\tt swap},
squential container ({\tt empty}, {\tt emplace\_back}, {\tt front}, {\tt pop\_back}, {\tt erase}, {\tt clear}),
string ({\tt NULL}, rational operators, {\tt find}, {\tt substr}, {\tt stoul}, {\tt npos}),
vector ({\tt data}, {\tt reserve}),
associative container ({\tt find}, {\tt count}, {\tt emplace}, {\tt empty}), map ({\tt at}), unordered\_map, set,
...
lambda expression, {\tt strtol}, {\tt getline},
std (thread, mutex, unique\_lock, lock\_guard, condition\_variable),
{\tt nullptr}, {\tt noexcept}, {\tt override},
lvalue, rvalue, rvalue reference,
postponement, RAII, exception-safe (copy and swap)),
Objective-C,
Python (sequence type (list, tuple), mapping type (dictionary), natsorted, sorted, map, lambda, isinstance, ternary, not, in, import, from, parameter passing,
global, nested function, scoping, assert, hex, format,
finally, module, argparse, pickle, re, subprocess, binascii),
Jinja2 (template, custom filter, macro, include, filter (combine)), YAML (sequence, mapping, scalar),
XML, AMPL, MATLAB, script (bash, tcsh, expansion), Fortran, Verilog, VHDL, SQL %
...
Software for Open Networking in the Cloud (SONiC) (SubscriberStateTable, CONFIG\_DB,
ProducerTable, ConsumerTable, FLEX\_COUNTER\_DB,
Table, COUNTERS\_DB,
NotificationConsumer, APPL\_DB, COUNTERS\_DB, SelectableTimer,
flex counter architecture, PFC watchdog, watermark, ACL, swss warm-reboot),
...
\item[Applications:] regular expression (basic, extended, [:space]), Linux kernel programming, Linux system programming ({\tt select}, {\tt poll}, {\tt epoll}, {\tt getenv}, timerfd), interprocess communication (IPC), process scheduling,
multithreading (POSIX), synchronization \& condition synchronization, file access, iOS programming, Android programming,
OPNET simulator, AMPL/lp\_solve, AMPL/CPLEX, design patterns (observer, bridge (separate an abstraction from its implementation),
decorator (add responsibilities incrementally \& dynamically), singleton, publish-subscribe), Kusto query,
packet test framework (PTF) (dataplane),
redis ({\tt get}, {\tt hget}, {\tt set}, {\tt hset}, {\tt del}, {\tt hdel} (multiple fields)), Broadcom SDK (BST, KNET, QoS mapping, FP, LAG, hashing), OpenNSL
...
Jira, CodeFlow,
jumpbox, Dnsmasq, syslog

\item[Linux command line:] {\tt lsblk}, {\tt fdisk}, {\tt df}, {\tt du},
{\tt ps},
{\tt mkfs}, {\tt grep}, {\tt find}, {\tt less}, {\tt tail}, {\tt ln}, {\tt mount}, {\tt unmount}, /etc/fstab, {\tt file}, {\tt chmod},
{\tt tar} (.tar, .tar.gz), {\tt gzip}, {\tt gunzip} (.gz), {\tt zcat}, {\tt zgrep},
{\tt ifconfig}, {\tt ip addr}, {\tt ip link}, {\tt ethtool}, {\tt arp}, {\tt ip neigh}, {\tt ip link}, {\tt tcpdump}, {\tt ip route}, {\tt wget}, {\tt curl},
{\tt chown}, {\tt adduser}, {\tt usermod}, {\tt passwd}, {\tt id},
{\tt dpkg},
{\tt which},
{\tt lsusb},
{\tt virsh},
{\tt cat} (-A, ctrl-D, << token), {\tt awk}, {\tt cut} (-c, -f, -d), {\tt sort} (-k, -n), {\tt uniq}, {\tt sed} (-n)

\item[Hardware:] NetFPGA-10G, Xilinx, Virtex-5, Altera UDP/IP core, network interface card (Mellanox, Chelsio, Emulex),
switch (Arista 7124SX, 7050QX-32S, 7060CX-32S, 7260CX3-64, Dell S6100-ON, Mellanox SN2700,
...
cross-point (XP) architecture, PFC, ECN, WRED, mirroring, field processor (legacy, new), VRF, VXLAN, hashing (flex, resilient), ECMP (weighted, hierarchical),
user defined field (FP, hashing, flex hashing, flexible counter), flexible counters,
BCM88670 (Jericho), packet processor architecture),
...
fetch -{}-tags, push -{}-force -{}-delete, show, rebase, apply, cherry-pick, submodule update -{}-recursive, bisect),
...
\item
\textbf{Technical program committee member} of IEEE International Conference on Optical Network Design and Modeling (ONDM) 2013, 2014, 2015, 2016, 2017, 2018, 2019
...
Optical and Grid Computing Symposium 2014, 2015, 2016, 2017, 2018, 2019
...
\item
\textbf{Reviewer} of IEEE/ACM Transactions on Networking (1), IEEE/OSA Journal of Lightwave Technology (5),
