...
\item[Networking:] IEEE 802.3 (i.e., Ethernet), control plane (ioctl, AXI4-Lite, MDIO, I$^2$C), NetFPGA-10G, network interface card (NIC), network device driver, socket programming, bridging, TCP/IP, UDP, iWARP (RDMAP/DDP/MPA), InfiniBand, RoCE, RDMA, Verbs programming, Wireshark, OPNET simulator

\item[Linux kernel:] kernel architecture, interrupt context, interrupts and interrupt handlers, softirq, tasklet, kernel synchronization, process context, work queues, kernel threads and sleep (wait queues), kernel preemption, hrtimer, Network traffic control (TC), Tx \& Rx path below layer 3, new API (NAPI), char driver, memory management, kprobe
