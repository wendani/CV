...
\item
  ...
  \begin{itemize}
  \resitem{Topics: Lambda network operating system, a customized version of SONiC, over world's bleeding-edge switching chips (25G, 100G, 400G, 800G)}
  ...
  Applied separate PG to service pool mapping profile to CPU port under TH4/TD4 profile-id-based mapping architecture;
  ...
  \resitem{Evaluated and validated hardware capability to match VxLAN overlay 5 tuples on TH4 and TD4.}
  \resitem{In truncated packet sample case, pinpointed the root cause of incorrect frame length population in sFlow v5 raw packet header
  down to Broadcom knet psample callback (kernel 4.19.0-12-2), and developed fix.}

  \resitem{Enabled hosting user ACL tables in system COPP/trap field group to fully exploit hardware resource capacity.}
  \resitem{Enhanced SAI ACL with new match fields and actions: Match next hop/next hop group;
  Action to redirect packet to dynamic load balancing (DLB) next hop group;
  Action to enable adaptive routing and switching (ARS);
  Match RoCEv2 BTH reserved field using UDF yet not consuming additional container (i.e., leveraging the notion of UDF container sharing
  among orthogonal packet types).}
  \resitem{Pinpointed the root cause of transcient packet match on all ports at new ACL rule installation down to Broadcom SAI, and
  designed remedies to inhibit such a match for both ingress and egress ACLs.}
  \resitem{Ported ACL custom changes from Broadcom SAI 7.1 to 9.2, which undergoes fundamental architectural refactoring in ACL implementation.}
  \resitem{Validated egress sFlow hardware offload on TH5, leveraging egress flex sampler and egress pipeline recirculation (EPRC) newly introduced
  to Tomahawk chip family, and designed SAI implementation.}
  \resitem{Extended IFA 2.0 transit functionality to TH5.}
  \resitem{In PFC watchdog hardware detection software restoration scenario, migrated PFC storm action from software-managed ACL to hardware native
  storm action (with reference to public implementation). The migration incurrs minor changes (10 insertions, 4 deletions) on top of public implementation cherry-pick.
  Covered the overlooked case of in-storm PFC watchdog disable, at which original Broadcom SAI causes incorrect hardware state machine transition on storming queue
  post operation.}
  \resitem{\textcolor{Red}{Earned 2023 H2 Data-SYS Excellent Project Award (优秀项目奖)
  being a team member of project white-box switches for network infrastructure 4.0 (4.0 网络架构自研交换机项目).}}
  \resitem{\textcolor{Red}{White-box switches running Lambda OS reached the milestone of 10,000 production deployment at the end of November 2023.}}
  \resitem{Validated MMU setting and PFC watchdog behavior on TH4 against runtime TC change from lossy to lossless and back.}
  \resitem{Validated MMU setting on TH5, which follows the same design principles and practices set forth (by me) on TH4.}

  \resitem{Co-designed SAI model and header (object, attributes, event notification, etc.) for global load balancing (GLB) with Broadcom.}
  \resitem{Designed GLB message send scheme without the need for multicast entry.}
  \resitem{Implemented GLB SAI on Broadcom TH5. Customized HSDK notification data struct for remote link down event in multi-link scenario.}
  \resitem{\textcolor{Red}{Earned Broadcom 2024 Global Best Implementation Award of Tomahawk5 System, being a member of ByteDance Network Engineering team.}}
  \resitem{Presented GLB with Broadcom at OCP Global Summit 2024 on behalf of ByteDance white box team.}

  \resitem{Customized SAI logic, concerning create, remove, and set operations, for ACL entry with inports match field.}

  \resitem{Extended KNET psample callback logic on HSDK 6.5.30 (TH5) to cover packets that are marked both ingress and egress sampled.}
  \resitem{Removed pipeline-added outer tag (VLAN 4095) from sampled packets in KNET psample callback.}
  \resitem{Learned and developed kernel live patch to fix NLA padding bug in KNET psample callback for production devices.}

  \resitem{\textcolor{Red}{Earned 2024 Data-SYS Excellent Project Award in hardware R\&D (硬件研发领域优秀项目奖)
  being a team member of project B5020 white-box switches (B5020 自研交换机项目).}}

  \resitem{Migrated egress flex counter implementation from global to pipe instance unique mode
  to deal with counter index space explosion in TH4 256$\times$100G port breakout.}
  \resitem{Implemented prefix compression SAI on TH5.
  Designed and developed prefix overlapping check on top of Broadcom SAI trie data structure and utilities.}

  \resitem{Validated MMU profile setting for TD5.}
  \end{itemize}
...
BCM56788 (Trident4 X9), BCM56990 (Tomahawk4), BCM78900 (Tomahawk5), BCM78800 (Trident5 X12), BCM78910 (Tomahawk6),
...
mirroring (ERSPAN, local SPAN), mirror on drop (MOD),
...
hashing (static, robust, flex, consistent, resilient), dynamic load balancing (DLB), global load balancing (GLB),
flex digest, ECMP (weighted, hierarchical), LAG,
user defined field (FP, hashing, flex hashing, flexible counter), flexible counters (1.0, 2.0, 2.2), flex monitoring,
trace event, drop event,
packet sampling (ingress, egress, ingress flex sampler, egress flex sampler), sFlow, sFlow hardware offload, IFA (1.0, 2.0),
EP recirculation, drop congestion notification (DCN),
...
\item[Test:] pytest, Google test, Spirent test center (STC)
...
Optical and Grid Computing Symposium 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2023, 2024
...
\textit{Partner software engineering manager}, Azure Networking\\
